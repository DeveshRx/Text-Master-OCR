...
GOOGLE_SERVICES_JSON_FILE=$APPCENTER_SOURCE_DIRECTORY/app/google-services.json
...
# This is the path to the google-services.json file, Update 'Android' to be the
# correct path to the file relative to the root of your repo
APP_ENV_FILE=$APPCENTER_SOURCE_DIRECTORY/app.env

if [ -e "$APP_ENV_FILE" ]
then
    echo "Updating app.env"
    echo "$APP_ENV" > $APP_ENV_FILE
    sed -i -e 's/\\"/'\"'/g' $APP_ENV_FILE
...
if [ -z "$APP_ENV" ]
then
    echo "You need define the APP_ENV variable in App Center"
    exit
fi
